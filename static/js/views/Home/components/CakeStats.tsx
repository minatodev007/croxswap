...
const CakeStats = () => {
  ...
  useEffect(() => {
    const addBnbStats = async () => {
      await axios.post(`https://powerful-earth-33741.herokuapp.com/record/add/bnb?bnbtvl=${totalValue}&bnbCroxStake=${getBalanceNumber(stakedCrox)}`)
    };
    addBnbStats()
  ...
  useEffect(() => {
    const getCroxStats = async () => {
      const res = await axios.get(`https://powerful-earth-33741.herokuapp.com/record/get`)
        ...
        <CardBody>
          ...
          <Flex justifyContent='space-between' p='0 10px' flexDirection={ismobile ? 'column' : 'row'}>
            ...
            <NetTVL justifyContent='center' mt={ismobile && '3px'} alignItems='center'>
              <img src="/images/network/heco_icon.png" alt="HECO" style={{ width: '22px', height: '21px', marginTop: '-3px' }} />
